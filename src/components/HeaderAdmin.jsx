...
function HeaderUser() {
  ...
  return (
    <>
      <header className="flex py-[2vh] px-[15px] items-center bg-white justify-between md:py-[2vh] md:px-[3vw] xl:px-[5vw] select-none border-b border-[#e8e8e8]">
        <section className={"modal h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-black/75 z-20 " + (logoutModal ? "" : "hidden")}>
        ...
        <section className="left-head flex md:flex md:gap-[3.5vw]">
          <section className="coffee-logo">
            <Link to="/" className="flex gap-[15px] items-center">
              <img className="select-none" src={getImageUrl("Vector", "svg")} alt="coffee-shop-logo" />
              <h1 className="text-[#8E6447] font-sacramento select-none cursor-pointer text-xl font-normal">Coffee Shop</h1>
            </Link>
          </section>
        </section>
        ...
        <div className={" nav-down z-10 absolute top-[65px] right-[0px] w-screen h-screen p-5 bg-[#e8e8e8] flex flex-col gap-3 items-center lg:hidden " + (navShown ? "" : "hidden")}>
          <input type="text" placeholder="Search..." className="w-full text-white p-3 pl-10 rounded-md mb-4 bg-[#aeaeae] bg-[url(./assets/images/Search-black.svg)] bg-no-repeat bg-[center_left_10px] placeholder:text-white" />
          <Link to="/">
          ...
          <Link to="/checkout" className="flex gap-[15px] items-center mb-4">
            <img className="w-[24px] h-[24px] max-w-none" src={getImageUrl("ShoppingCart-black", "svg")} alt="shopping-cart" />
        ...
        <nav className="right-nav hidden lg:flex lg:items-center lg:gap-[22px]">
          ...
          <img className={"w-[24px] h-[24px] cursor-pointer " + (!searchBar ? "" : "hidden")} src={getImageUrl("Search-black", "svg")} alt="search" onClick={searchBarHandler} />
          <Link to="/checkout">
            <img className="cursor-pointer w-[24px] h-[24px]" src={getImageUrl("ShoppingCart-black", "svg")} alt="shopping-cart" />
          </Link>
          ...
          <div className={"hidden relative lg:flex items-center gap-2 " + (isUser ? "" : "lg:hidden")}>
            ...
            <div className=" cursor-pointer">
              <img src={getImageUrl("down-black", "svg")} alt="drop-down" onClick={dropDownHandler} />
            </div>
            <div className={"absolute z-10 flex flex-col gap-3 items-center bottom-[-160px] right-0 p-4 text-black bg-white rounded-sm w-[250%] shadow-xl " + (dropDown ? "" : "hidden")}>
              <Link to="/history">
